        ...
        graph.stream().forEach(v -> {
            ...
            Map<Integer, Long> neighbourPartitions =
                    v.neighs() //get v's direct neighbours
                            ...
                            .filter(p -> !p.partitions().isEmpty()) //filter out neighbours that were not partitioned yet
                            .flatMap(p -> p.partitions())
                            .collect(Collectors.groupingBy(, Collectors.counting())); //count number of neighbours in each partition
